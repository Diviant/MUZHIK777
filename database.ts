...
class MuzhikDatabase {
  ...
  async saveUser(user: User): Promise<void> {
    ...
    await supabase.from('profiles').upsert([{
      ...
      // Fix: referral_code property in the profile table should map to user.referralCode from the User object
  ...
  // Fix: Correct city_id mapping to cityId for MarketItem
  ...
  async addHitchhiker(h: Hitchhiker): Promise<void> { 
    await supabase.from('hitchhikers').insert([{ 
      author_id: h.authorId, 
      name: h.name, 
      route_from: h.routeFrom, 
      route_to: h.routeTo, 
      departure_date: h.departureDate, 
      price: h.price, 
      car_model: h.carModel, 
      seats: h.seats, 
      description: h.description, 
      contact: h.contact,
      can_take_cargo: h.canTakeCargo
    }]); 
  }
