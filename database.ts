...
class MuzhikDatabase {
  // Проверка на валидный UUID для Supabase
  ...
  async sendSOS(userId: string, userName: string, signal: Omit<SOSSignal, 'id' | 'userId' | 'userName' | 'status'>): Promise<void> {
    ...
    await supabase.from('sos_signals').insert([sosData]);
    const scenarioLabels: Record<string, string> = {
      'ACCIDENT': 'ДТП', 'INJURY': 'ТРАВМА НА ОБЪЕКТЕ', 'STUCK': 'ЗАСТРЯЛ В ПУТИ', 'THREAT': 'УГРОЗА / КОНФЛИКТ', 'OTHER': 'НУЖНА ПОМОЩЬ'
    };
    const content = `[SOS] СРОЧНО! Требуется помощь: ${scenarioLabels[signal.scenario] || 'Неизвестная ситуация'}. Мои координаты: ${signal.lat.toFixed(4)}, ${signal.lng.toFixed(4)}.`;
    await this.addFeedPost(userId, content);
  }

  async updateTrustedContacts(userId: string, contacts: string[]): Promise<void> {
    if (!this.isValidUuid(userId)) return;
    await supabase.from('profiles').update({ trusted_contacts: contacts }).eq('id', userId);
  }
  ...
  async addFeedPost(userId: string, content: string, imageUrl?: string): Promise<void> { 
    if (!this.isValidUuid(userId)) return;
    await supabase.from('feed_posts').insert([{ author_id: userId, content, image_url: imageUrl }]); 
  }
